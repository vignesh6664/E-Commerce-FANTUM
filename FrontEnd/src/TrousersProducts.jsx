...
const TrousersProducts = () => {
  ...
  useEffect(() => {
    axios.get("http://localhost:3000/pants/" + id).then((res) => {
  ...
  return (
    ...
    <div className="grid lg:grid-cols-2">
      ...
      <div className="mt-9">
        <h1 className="font-bold text-5xl md:ml-20">{trousers.name}</h1>
        <p className="text-xl mt-4 font-normal md:ml-20">Rs. {trousers.price}.00</p>
        ...
        <div>
          ...
          <div className="flex gap-7 border-2 w-[150px] justify-center p-3 font-bold text-lg mt-4 md:ml-20">
            <span onClick={decrement}>-</span> <p>{quantity}</p> <span onClick={increment}>+</span>
          </div>
        </div>
